    ...
    async function f() {
      ...
      ChatCompletionStream.fromReadableStream(stream)
        .on("content", (delta, content) => {
          ...
          if (
            !didPushToCode &&
            splitByFirstCodeFence(content).some((part) => part.type === "first-code-fence-generating")
          ) {
            didPushToCode = true
          ...
          if (!didPushToPreview && splitByFirstCodeFence(content).some((part) => part.type === "first-code-fence")) {
    ...
    <div className="h-dvh">
      <div className="flex h-full">
        <div className="mx-auto flex w-full shrink-0 flex-col overflow-hidden lg:w-1/2">
          ...
          <ChatBox chat={chat} onNewStreamPromise={setStreamPromise} isStreaming={!!streamPromise} />
        </div>
