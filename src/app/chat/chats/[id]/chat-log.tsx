...
export default function ChatLog({
  ...
  return (
    <StickToBottom className="relative grow overflow-hidden" resize="smooth" initial="smooth">
      <StickToBottom.Content className="mx-auto flex w-full max-w-prose flex-col gap-8 p-8">
          ...
          <Fragment key={message.id}>
            {message.role === "user" ? (
              ...
              <AssistantMessage
                ...
                version={assistantMessages.map((m: Message) => m.id).indexOf(message.id) + 1}
                message={message}
        ...
        {streamText && <AssistantMessage content={streamText} version={assistantMessages.length + 1} isActive={true} />}
...
function UserMessage({ content }: { content: string }) {
  return (
    <div className="relative inline-flex max-w-[80%] items-end gap-3 self-end">
      <div className="whitespace-pre-wrap rounded bg-white px-4 py-2 text-gray-600 shadow">{content}</div>
    </div>
  )
}
        ...
        <div key={i}>
          {part.type === "text" ? (
            ...
            <div className="my-4">
              ...
              >
                <div className="flex size-8 items-center justify-center rounded bg-gray-300 font-bold">V{version}</div>
                <div className="flex flex-col gap-0.5 text-left leading-none">
                  <div className="text-sm font-medium leading-none">Generating...</div>
                </div>
              </button>
            </div>
          ) : message ? (
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 text-left leading-none">
                  <div className="text-sm font-medium leading-none">
                    {toTitleCase(part.filename.name)} {version !== 1 && `v${version}`}
                  </div>
