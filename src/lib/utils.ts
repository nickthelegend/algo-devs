  ...
  for (const line of lines) {
    ...
    if (!codeFenceFound) {
      if (match && !inFirstCodeFence) {
        ...
        const parsedFilename = extractedFilename ? parseFileName(extractedFilename) : { name: "", extension: "" }

        // Extract language from the portion of fenceTag before '{'
        const bracketIndex = fenceTag.indexOf("{")
        const language = bracketIndex > -1 ? fenceTag.substring(0, bracketIndex).trim() : fenceTag.trim()
